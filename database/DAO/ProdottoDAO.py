...
class ProdottoDAO:
    ...
    def insert(self, asin: str, titolo: str, prezzo: float, old_prezzo: float, valuta: str, sconto: float, venditore: str, spedito_Amazon: bool, link: str, img_url: str, brand: str, preorder: bool, data_preordine: str, isPrime: bool, isWarehouse: bool, condizione: str, condizione_descrizione: str, last_check: int, priorita: int) -> None:
        self.cursor.execute('''INSERT INTO prodotti VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', 
                            (asin, titolo, prezzo, old_prezzo, valuta, sconto, venditore, spedito_Amazon,
                             link, img_url, brand, preorder, data_preordine, isPrime, isWarehouse, condizione, condizione_descrizione, last_check, priorita))
        self.conn.commit()
